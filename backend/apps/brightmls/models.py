...
class BaseModel(models.Model):
    """
    Base model that provides a generic from_pyodata method
    to map data from a pyodata entity to Django model fields.

    usage: instance = AnyModel.from_pyodata(podata_entity_object)
    """
    ...
    @classmethod
    def from_pyodata(cls, pyodata_obj):
        """
        Generic method to map pyodata entity fields to Django model fields.
        Automatically matches fields based on name.
        """
        field_values = {}
        for field in cls._meta.get_fields():
            field_name = field.name
            # Map pyodata field to the model field if it exists in the pyodata object
            pyodata_field = cls._map_field_name_to_pyodata(field_name)
            if pyodata_field in pyodata_obj:
                # Parse datetime fields if necessary
                if isinstance(field, models.DateTimeField):
                    field_values[field_name] = parse_datetime(
                        pyodata_obj[pyodata_field]
                    )
                else:
                    field_values[field_name] = pyodata_obj[pyodata_field]
            elif isinstance(field, models.ForeignKey):
                # Handle foreign key relationships
                related_model = field.related_model
                related_key = pyodata_obj.get(pyodata_field)
                if related_key:
    ...
    @staticmethod
    def _map_field_name_to_pyodata(field_name):
        """
        Map Django model field names (snake_case) to pyodata field names (CamelCase).
        """
...
class City(BaseModel):
    ...
    cty_city_towhnship = models.IntegerField()
    ...
    @staticmethod
    def endpoint_list():
        return "RESO/OData/bright/City"

    @staticmethod
    def endpoint_detail(city_key):
        return f"RESO/OData/bright/City({city_key})"
...
class CityZipCode(BaseModel):
    ...
    def __str__(self):
        return self.city_zip_code_city_name
...
class TeamMember(BaseModel):
    ...
    def __str__(self):
        return f"Team Member {self.team_member_key}"
...
class RelatedLookup(BaseModel):
    ...
    related_lookup_key = models.CharField(max_length=15, null=True)
    modification_timestamp = models.DateTimeField(null=True)
