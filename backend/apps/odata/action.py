...
class ActionCallable(object):
    ...
    def __call__(self, **kwargs):
        ...
        return self.actionbase_instance._callable(
            connection, self.url, self.query, **kwargs
        )
        ...
        if self.bound_to_collection is False:
            if instance is None:
                ...
                if es.persisted is False:
                    errmsg = (
                        "Trying to call instance bound Action or "
    ...
    def _callable(self, connection, url, query, **kwargs):
        ...
        if self.return_type_collection:
            ...
            for value in response_data or []:
                entity_instance = self.return_type_collection.__new__(
                    self.return_type_collection, from_data=value
                )
        ...
        if self.return_type:
            ...
            entity_instance = self.return_type.__new__(
                self.return_type, from_data=response_data
            )
