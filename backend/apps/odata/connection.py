...
class ODataConnection(object):
                ...
                if "exception" in errordata:
                    ...
                    for candidate in inner:
                        if candidate in odata_exception:
                            ...
                            detailed_message = (
                                ie.get("message", None) or detailed_message
                            )
                else:
                    detailed_message = response.headers.get(
                        "WWW-Authenticate", detailed_message
                    )
            else:
                detailed_message = response.text

            msg = " | ".join(
                [str(status_code), str(code), str(message), str(detailed_message)]
            )
            err = ODataError(msg)
    ...
    def execute_get(
        self, url, params=None, allow_plain_response=False, extra_headers=None
    ):
        headers = {}
    ...
    def execute_post(
        self, url, data, raw: bool = False, params=None, extra_headers=None
    ):
        headers = {
