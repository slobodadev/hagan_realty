...
class MetaData(object):
    ...
    def __init__(self, service, console: rich.console.Console, quiet: bool = False):
    ...
    def _get_entities_from_types(self, all_types):
        return [
            entity for entity in all_types.values() if issubclass(entity, EntityBase)
        ]

    def _set_object_relationships(self, all_types):
        entities = self._get_entities_from_types(all_types)
        for entity in rich.progress.track(
            entities,
            ...
            for schema_nav in all_schema_navs:
                ...
                for _search_entity in entities:
                    ...
                    _search_type_alias = _search_entity.__odata_schema__.get(
                        "type_alias"
                    )
    ...
    def _create_entities(self, all_types, entity_base_class, schemas, depth=1):
        ...
        with rich.progress.Progress(
            transient=True, console=self.console, disable=self.quiet
        ) as progress:
            schema_task = progress.add_task("Schemas", total=len(schemas))
            for schema in schemas:
                entity_task = progress.add_task(
                    f"Creating entities for {schema['name']}",
                    total=len(schema.get("entities")),
                )
                for entity_dict in schema.get("entities"):
                    ...
                    for prop in entity_dict.get("properties"):
                        ...
                        if property_type and issubclass(property_type, EnumType):
                            property_instance = EnumTypeProperty(
                                prop_name, enum_class=property_type
                            )
                            property_instance.is_computed_value = prop[
                                "is_computed_value"
                            ]
                        else:
            ...
            if len(orphan_entities) > 0:
                if depth > 10:
                    errmsg = (
                        "Types could not be resolved. " "Orphaned types: {0}"
                    ).format(", ".join(orphan_entities))
    ...
    def _create_actions(self, all_types, actions, get_entity_or_prop_from_type):
        entities = self._get_entities_from_types(all_types)
        for action in rich.progress.track(
            actions,
            "Creating actions",
            console=self.console,
            transient=True,
            ...
            for param in action["parameters"]:
                parameters_dict[param["name"]] = self.property_type_to_python(
                    param["type"]
                )
            ...
            object_dict = dict(
                ...
                return_type_collection=get_entity_or_prop_from_type(
                    action["return_type_collection"]
                ),
    ...
    def _create_functions(self, all_types, functions, get_entity_or_prop_from_type):
        entities = self._get_entities_from_types(all_types)
        for function in rich.progress.track(
            functions,
            "Creating functions",
            console=self.console,
            transient=True,
            ...
            object_dict = dict(
                ...
                return_type_collection=get_entity_or_prop_from_type(
                    function["return_type_collection"]
                ),
                bound_to_collection=bound_to_collection,
            )
            function_class = type(
                function["name"], (self.service.Function,), object_dict
            )
    ...
    def get_entity_sets(self, base=None):
        ...
        with rich.progress.Progress(
            ...
            schema_task = progress.add_task("Schemas", total=len(schemas))
            for schema in schemas:
                enum_task = progress.add_task(
                    f"Enums for {schema['name']}", total=len(schema["enum_types"])
                )
        ...
        sets = {}
        for entity_set in rich.progress.track(
            entity_sets.values(),
            ...
            set_class = type(
                "EntitySet" + set_name,
                (entity_class,),
                dict(__odata_collection__=set_name, __odata_singleton__=is_singleton),
            )
        ...
        self.log.info(
            "Loaded {0} entity sets, total {1} types".format(len(sets), len(all_types))
        )
        ...
        for schema in xmlq(doc, "edmx:DataServices/edm:Schema"):
            ...
            for entity_type in xmlq(schema, "edm:EntityType"):
                entity = self._parse_entity(
                    xmlq, entity_type, schema_name, schema_alias
                )
