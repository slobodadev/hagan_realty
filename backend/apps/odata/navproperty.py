...
class NavigationProperty(object):
    ...
    def __init__(
        self,
        name,
        entitycls: Union[type, str],
        entity_package: str = None,
        collection=False,
        foreign_key=None,
    ):
    ...
    def __repr__(self):
        return "<NavigationProperty to {0}>".format(self.entitycls)

    def __populate_entity(self, data, connection, parent_navigation_url):
        result = self.entitycls.__new__(
            self.entitycls, from_data=data, connection=connection
        )
        es = result.__odata__
    ...
    def entitycls(self):
        ...
        if isinstance(self.entityclass, str):
            if not self.class_package:
                raise ODataReflectionError(
                    "Entitycls is a string, if you specify the entity class as a string you also need to specify "
    ...
    def __get__(self, instance, owner):
        ...
        if self.is_collection:
            if "collection" not in cache:
                ...
                if raw_data:
                    cache["collection"] = self.instances_from_data(
                        raw_data["value"], connection, nav_url
                    )
                else:
                    cache["collection"] = []
