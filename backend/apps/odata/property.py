...
from .navproperty import (
    NavigationProperty,
)  # actually used by other imports, don't remove
...
class BaseQueryFilter(object):
    ...
    def __and__(self, other: "BaseQueryFilter"):
        if isinstance(other, BaseQueryFilter):
            ...
            raise ODataQueryError(
                "Can only use other PropertyBase classes for & (AND) operation"
            )
    ...
    def __or__(self, other: "BaseQueryFilter"):
        if isinstance(other, BaseQueryFilter):
            ...
            raise ODataQueryError(
                "Can only use other PropertyBase classes for | (OR) operation"
            )
...
class PropertyBase(object):
    ...
    def __init__(
        self, name, primary_key=False, is_collection=False, is_computed_value=False
    ):
        """
        :type name: str
    ...
    def lacks(self, value):
        """Does not contain"""
        value = self.escape_value(value)
        return UnaryQueryFilter(
            "not", ParameterizedQueryFilter(self.name, "contains", value)
        )
