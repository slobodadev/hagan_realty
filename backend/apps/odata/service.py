...
class ODataService(object):
    ...
    def __init__(
        ...
        )  # make sure url ends with / otherwise we have problems
        ...
        self.console = (
            ...
            else rich.console.Console(quiet=quiet_progress)
        )
        ...
        if reflect_output_package and base is None:
            try:
                ...
                if not quiet_progress:
                    self.console.print(
                        f"[red]Couldn't get ReflectionBase instance from {reflect_output_package} package."
                    )
        ...
        if reflect_entities:
            _, self.entities, self.types = self.metadata.get_entity_sets(
                base=self.Entity
            )
            if reflect_output_package:
                self._write_reflected_types(
                    metadata_url=self.metadata_url, package=reflect_output_package
                )
