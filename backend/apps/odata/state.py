...
class EntityState(object):
    def __init__(self, entity):
        ...
        self.parent_navigation_url: Optional[
            str
        ] = None  # for chaining objects, like OrderDetails.Order.Employee
    ...
    def describe(self):
        ...
        panel = rich.panel.Panel(
            ...
            subtitle=f"URL={self.instance_url or self.entity.__odata_url__()}",
            expand=False,
        )
        ...
        for _, prop in self.properties:
            ...
            show_properties.append(
                rich.console.Text.assemble(
                    rich.console.Text(name),
                    ": ",
                    rich.console.Text(
                        type(prop).__name__, style="dim blue", overflow="ellipsis"
                    ),
                )
            )
        ...
        for _, prop in self.navigation_properties:
            ...
            show_nav_properties.append(
                rich.console.Text.assemble(
                    name,
                    ": ",
                    rich.console.Text(prop.entitycls.__name__, style="dim blue"),
                    overflow="ellipsis",
                )
            )
        ...
        for items in itertools.zip_longest(
            show_properties, show_nav_properties, fillvalue=""
        ):
    ...
    def id(self):
        ...
        if len(ids) == 1:
            key_value = ids[0][1]
            return "{0}({1})".format(entity_name, key_value)
        if len(ids) > 1:
            key_ids = []
            for prop, key_value in ids:
                key_ids.append("{0}={1}".format(prop.name, key_value))
            return "{0}({1})".format(entity_name, ",".join(key_ids))
